...
public class No26RemoveDuplicates {
    public int removeDuplicates(int[] nums) {
        ...
        while (i > 0 ){
            if (nums[i - 1] == nums[i]){
                removeItemByIndex(nums, i, length);
                length--;
            }else {
                i--;
            }
     ...
     */
    public int battleSolution(int[] nums){
        if (nums.length == 0){
            return 0;
        }
        int i = 0;
    ...
    private void removeItemByIndex(int[] nums, int index, int arrayLength){
        for (; index < arrayLength - 1; index++){
