...
public class No28strStr {
    public int strStr(String haystack, String needle) {
        ...
        int[] next = getNextArray(needleChars);
        int i = 0;
        int k = 0;
        while(i < haystackChars.length && k < needleChars.length){
            if (k == -1 || haystackChars[i] == needleChars[k]){
                i++;
                k++;
            }else {
                k = next[k];
    ...
    private int[] getNextArray(char[] needleChars){
